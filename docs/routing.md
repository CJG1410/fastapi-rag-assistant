...
### Configure the app `entrypoint` in `pyproject.toml`[¶](#configure-the-app-entrypoint-in-pyproject-toml "Permanent link")
...
### `fastapi dev` with path or with `--entrypoint` CLI option[¶](#fastapi-dev-with-path-or-with-entrypoint-cli-option "Permanent link")
